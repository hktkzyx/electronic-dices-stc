...
__code u8 kLEDCoding[16] = {0x3F, 0x06, 0x5B, 0x4F, 0x66,
                            0x6D, 0x7D, 0x07, 0x7F, 0x6F,
                            0x77, 0x7C, 0x39, 0x5E, 0x79, 0x71}; // 0 to F

#ifndef kDiceMaxNum
#define kDiceMaxNum 4
#endif

u8 leds[kDiceMaxNum] = {1};
u8 dices_num = 1, setting_status = 1, rolling_status = 1; // Key status 0: pressed, 1: unpressed
...
void SettingKey(void)
{
    ...
    if ((setting_backup == 0) && (setting_status == 1)) // Setting key is released
    {
...
void RollingKey(void)
{
    ...
    u8 i = 0;
    if ((rolling_backup == 1) && (rolling_status == 0)) // Rolling key is pressed
    {
        for (i = 0; i < dices_num; i++) // Display "8" when pressing rolling
        {
            leds[i] = 8;
        }
        rolling_backup = 0;
...
void InterruptIT0(void) __interrupt(0)
{
    nop();
}
void InterruptIT1(void) __interrupt(2)
{
    nop();
}
